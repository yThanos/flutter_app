...
class LoteDAO{
  adicionar(Lote l) async{
    ...
    await http.post(Uri.parse("http://10.0.2.2:8080/lote"),headers: header, body: jsonEncode(l.toMap()));
  }

  Future<List<Lote>> getLotes() async {
    String token = await getToken();
    final header = {'Content-Type': 'application/json', 'Authorization': 'Bearer $token'};
    final response = await http.get(Uri.parse("http://10.0.2.2:8080/lote"),headers: header);
